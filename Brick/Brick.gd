...
func _ready():
	...
	color_initial_position = $Sprite.position
...
func _physics_process(_delta):
	if dying and not $Confetti.emitting and not $Tween.is_active():
		...
		if Global.color_rotate >= 0:
			$Sprite.modulate = colors[(int(floor(color_distance + Global.color_rotate))) % len(colors)]
		...
		$Sprite.position = Vector2(color_initial_position.x + pos_x, color_initial_position.y + pos_y)
